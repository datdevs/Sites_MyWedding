...
function nv_site_theme($contents, $full = true)
{
    ...
    $html_js[] = [
        'ext' => 1,
        'content' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/js/app.min.js'
    ];
    $html_js[] = [
        'ext' => 1,
        'content' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/js/contact.js'
    ];
    ...
    // $API_KEY = 'AIzaSyCBCFjBMERjN2MBBg87N3T-XUJ21bqGMBI';

    // $getAlbum = file_get_contents(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/album/engaged.json');
    // $album_decode = json_decode($getAlbum);

    // if (!empty($album_decode)) {
    //     foreach (array_slice($album_decode->items, 0, 6) as $item) {
    //         $id_photo = $item->id;
    //         $getPhoto = file_get_contents('https://www.googleapis.com/drive/v3/files/' . $id_photo . '?fields=name%2C%20webContentLink&key=' . $API_KEY);
    //         $photo_decode = json_decode($getPhoto);
    //         $xtpl->assign('PHOTO_NAME', $photo_decode->name);
    //         $xtpl->assign('PHOTO_URL', $photo_decode->webContentLink);
    //         $xtpl->parse('main.gallery.loop');
    //     }
    ...
    //   <a href="{PHOTO_URL}" class="img-zoom">
    //     <div class="gallery-box">
    //       <div class="gallery-img"> <img src="{PHOTO_URL}" class="img-fluid mx-auto d-block" alt="{PHOTO_NAME}">
    //       </div>
